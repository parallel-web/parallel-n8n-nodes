...
export class Parallel implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			{
				displayName: 'Maximum Wait (Minutes)',
				name: 'syncWaitMinutes',
				type: 'number',
				typeOptions: { minValue: 1, maxValue: 120 },
				default: 75,
				displayOptions: { show: { resource: ['task'], operation: ['webEnrichment'] } },
				description:
					'Total time to wait for an existing Task result. The n8n execution limit may be shorter. Use Async for long tasks.',
			},
			{
			...
			// List Monitor Events additional fields
			{
				...
				options: [
					...
					{
						displayName: 'Legacy Lookback Period',
						name: 'lookbackPeriod',
						type: 'string',
						default: '',
						description:
							'Unsupported by the current API. Remove this saved option and use pagination; older history may be unavailable.',
					},
					{
						displayName: 'Limit',
						name: 'limit',
